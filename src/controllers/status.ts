...
export let create_new_acc = (req: Request, res: Response) => {
    //let sql = `INSERT INTO accounts (account_name, owner_user_id, balance, last_updated) VALUES ('${req.body.account_name}', '${req.body.owner_user_id}', '${req.body.balance}', DEFAULT)`;
...
export let get_user_by_user_id = (req: Request, res: Response) => {
    ...
    dbFunctions.query(sql, (err, result) => {
        if (err) 
            res.status(500).json(err)
        else
            res.json(result)
