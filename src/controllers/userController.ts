...
export async function readUserById (req: Request, res: Response) {
  ...
  if (isAuthorized(req.app.locals.authData, parseInt(req.params.id, 10))) {
    try {
      const userById = await retrieveUserById(parseInt(req.params.id))
...
export async function updateUser (req: Request, res: Response) {
  ...
  if (isAuthorized(req.app.locals.authData, parseInt(req.params.id, 10))) {
    ...
    try {
      const userExists = await retrieveUserById(parseInt(req.params.id))
      if (userExists) {
         // admin access
  ...
  if (isAuthorized(req.app.locals.authData, parseInt(req.params.id, 10))) {
    try {
      ...
      if (userExists && !userExists.deletedAt) {
        const result = await removeUser(parseInt(req.params.id))
        if (result) {
          const returnObject = {
            id: result.id,
            userName: result.userName,
